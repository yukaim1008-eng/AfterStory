...
Phase 5：MemoriesPage 合并聊天片段与她记得的事，保持历史/记忆旧链接可用。历史卡片只使用真实预览、角色、轮数和时间；记忆只展示用户主动保存的内容。筛选展开分页时会加载后续页；角色或实例切换立刻清空旧记忆，并丢弃迟到响应。未安装角色、空记录、能力未启用与接口失败都有各自状态，不使用虚构数据补位。
