...
真实检查需单独终端设置 $env:DEV_USER_ID='frontend-smoke'，执行 seed，再将后端启动于8001。前端目录执行 node scripts/smoke-real.mjs。它手动调用一次当前模型，产生少量费用，使用独立身份，不写入默认用户。测试后关闭该独立进程，保留8000与前端供试用。
