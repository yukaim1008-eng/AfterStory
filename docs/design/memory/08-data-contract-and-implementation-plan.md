...
普通后台新增记忆与纠正/删除的失效语义分开：所有写入都推进供缓存和索引使用的数据修订；只有纠正、删除、抑制，或确实推翻本轮已选依据的变化，才推进用于拒绝旧回复的安全修订。现有 `context_revision` 暂承担后者，不能让每条自动新增记忆都无条件使正在生成的回复失败；具体字段迁移在 M1 验证。
...
原始消息的时间也是不可事后猜回的来源资料。新用户消息需记录服务端接收时间（UTC）、当时用于解释相对日期的用户时区及其来源/可靠性；回复完成时间另记，不能覆盖用户告知时间。客户端提供的时区应标为客户端报告，不把它当成外部核实。旧消息若只有 `Turn.created_at`，可标为近似告知时间；时区未知时不从“昨天”推算精确日期，也不以迁移或后台整理时间补齐。
...
- 摘要、事件提取和索引分别记录已成功处理的来源位置与缺口；进度只在结果提交后推进，不能用一个最大序号掩盖较早的失败任务。
...
对话中的“记住/纠正/忘记/设提醒”另有一条优先于回复承诺的显式操作路径：先持久化用户请求及稳定操作 ID，在锁外识别目标和歧义；能执行时用短事务提交操作并取得成功/失败回执，再将回执交给回复生成。目标不明确时只请求澄清，不先修改数据；角色不能凭模型意图识别结果自行宣称操作成功。若操作已提交而回复 Provider 失败，回执仍须可查询，重试同一操作 ID 返回原结果而不重复执行。“忘记”请求中重复提及的旧内容不能再作为普通自动提取来源；回复只使用必要的操作结果，不从被抑制的旧材料重新组装上下文。手动管理 API 继续独立可用；提醒操作在 M5 接入同一回执边界。

`prepare_context` 保存所选材料的安全修订和来源版本；`finish_turn` 对纠正/删除等安全变化拒绝旧结果。仅新增且不推翻所选依据的后台记忆不使旧回复失效，但会推进独立的数据修订，令下一轮检索及缓存看到新内容。若无法证明一次变化与本轮所选依据无关，则保守拒绝；不得以降低失败率为由放行删除前的回复。
...
- 显式对话操作按稳定操作 ID 提供可查询回执；回复失败时也能查看已提交结果，具体路由在 M3 定稿。
...
M1 同时补齐新消息的服务端接收时间与时区来源字段，以及缓存/索引数据修订与回复安全修订的区分。旧时间缺项只标记未知或近似，不通过迁移推断用户当时所在时区。
...
- 样本覆盖显式操作提交成功但回复失败、后台新增与回复并发、删除时大量派生依赖、跨会话未处理原文以及相对日期缺失时区；先定义各自应有的失败或降级结果。
- 不改变线上聊天行为。
...
- 建立有界的同步可见性屏障和旧 `history_floor_revision` 回退；依赖遍历与派生清理放在后台，安全修订与普通数据修订分开。
- 为新消息保存服务端接收时间、当时的用户时区及来源；旧消息保留时间不确定标记。
- 验收：旧数据不丢失；显式 CRUD 兼容；任务幂等；删除后所有新读取路径不可见；派生项很多时屏障事务仍有界；普通后台新增不无条件废弃在途回复。

### M2：分段摘要、连续性笔记与长会话预算

- 实现提前压缩、覆盖游标、续聊材料、近期原文和硬上限等待/重试；同时提供同实例、受删除边界约束的有界原文查找与回读，支持关键词/时间/消息引用定位，不依赖 M4 的向量检索。
- 对同实例其他会话中尚未被摘要/索引覆盖的成功轮次做有界补查；超过补查上限时等待整理或说明无法确认，不把“未检索到”说成“从未提及”。
- 调整 prepare/reserve/finish 边界，消除持锁网络调用风险。
- 验收：数百轮模拟、跨会话续接、旧原话准确回读、摘要积压时的跨会话补查、摘要失败和重启恢复、无静默截断；`begin_turn` 不再全量加载本会话 Turn。

### M3：事实、偏好与经历自动整理

- 实现后台增量提取、事件稳定身份、时间精度、两层去重和安全范围内的版本更新；复用 M2 的有界原文定位，并补充按主体/事项/时间查询已有记忆与事件候选。M4 再增强语义候选，不以向量可用作为去重前提。
- 显式“记住/纠正/忘记”使用提交回执；自动候选若需推翻旧来源、且当前只能依赖全实例 `history_floor_revision`，先留待澄清/确认，不高频自动清空长会话连续性。细粒度自动替代在 M6 完成依赖重建后开放。
- 验收：同事件跨批次不重复；推测/假设不污染事实；变化与纠错分开；保存失败不宣称成功；自动新增与回复并发不造成连续的 `context_changed`。

### M4：混合召回与 Runtime Context

- 在 M2 原文查找、M3 事件候选基础上接入结构化/全文候选、Embedding Provider、pgvector、重排和更丰富的原文回读。
...
- 扩展依赖传播、摘要/索引重建和 span 级来源；逐步减少粗粒度历史丢失，并在验证后开放 M3 暂缓的自动纠错/替代。
- 旧数据或依赖不完整时继续保守回退。
