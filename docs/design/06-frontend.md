...
2026-09-16 角色氛围文案精修：聊天页左上只显示白色 `AfterStory` 与主题色爱心；角色空间入口采用错位图标、不同长度细分隔线和轻量位移，签名及玻璃内两条批注使用系统楷体回退栈、不同角度和非对称位置。角色头像旁增加低对比个性短句。所有内容通过角色配置的 `sceneDecorations` 独立定义，支持 `avatarNote`、`leftMenu`、`signature`、`topNote`、`bottomNote` 以及文案的旋转、位置、透明度、颜色、字体风格；未改变 Chat Glass 尺寸、消息/输入逻辑或角色背景切换机制。
