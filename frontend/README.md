...
当前为已完成 UI / UX 改版的 F1/F2 可运行前端。一级入口为首页、角色、回忆、设置；聊天是选定角色的沉浸式二级空间。历史和用户主动管理的个人记忆已经接入；暂缓性格调整和角色资料库建设，语音等用户认可聊天效果后再做。完整 V1 尚未验收。见 [当前状态](../docs/current-status.md)。
...
- 三位主角色主题、角色化首页、可扩展角色选择、沉浸式聊天、资料、回忆和四类设置。
- 真实文字发送、失败重试、草稿保留、刷新恢复与历史分页加载。
- 字体、发送快捷键、动效偏好；封面上传、两处独立裁切、取消、保存与恢复默认。
- 个人记忆可手动添加或从成功用户消息保存，并可更正、删除、跳回来源。
- 语音仍显示尚未启用；剧情进度待补全，不模拟接口成功。
...
当前视觉依据是 `design/GPT设计图稿/` 中逐张审阅过的首页、角色、回忆和设置参考，以及用户给出的聊天空间描述。实现使用角色插画、主题 token、渐变、柔光和玻璃层近似场景，不把参考截图铺作背景。范围与验收见 [06 前端](../docs/design/06-frontend.md) 和 [UI 改版执行记录](../docs/implementation/ui-redesign.md)。
